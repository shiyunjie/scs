...
class UploadPage extends Component {
    ...
    _upload (uploadPhoto) {
        ...
        xhr.onload = () => {
            ...
            if (xhr.status == 200 && xhr.readyState == 4) {
               ...
               /* this.gunZip(xhr.responseText).then((response)=>{
                    let file={uri:uploadPhoto.uri,id:response.result}
        ...
        //formdata.append('file', {uri: uploadPhoto.uri, type: 'multipart/form-data', name:uploadPhoto.filename}); //for android, must set type:'...'
        formdata.append('text',uploadPhoto.filename);
        //这里增加其他参数, 比如: itype
        formdata.append('s', options.data.s);
        formdata.append('sign', options.data.sign);
        xhr.open('post', constants.api.service,true)
